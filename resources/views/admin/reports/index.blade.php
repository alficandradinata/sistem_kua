...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">Laporan Rekap Reservasi</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-6xl space-y-6 sm:px-6 lg:px-8">
            ...
            <div class="rounded-lg bg-white p-5 shadow-sm">
                <form method="GET" class="grid gap-4 sm:grid-cols-4">
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500">Jenis laporan</label>
                        <select name="type"
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500">Tanggal acuan</label>
                ...
                <div class="mt-5 rounded-lg border border-indigo-100 bg-indigo-50 p-4">
                    ...
                    <div class="mt-3 grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
                        ...
                        @foreach (\App\Models\Reservation::STATUSES as $status => $label)
                            <div>
                                <p class="text-xs text-indigo-700">{{ $label }}</p>
                                <p class="text-2xl font-bold text-indigo-900">{{ $preview[$status] }}</p>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.reports.store') }}" class="mt-4">
                        ...
                        <span class="ml-2 text-xs text-indigo-700">
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                ...
                @if ($reports->hasPages())
                    <div class="border-t border-gray-100 px-5 py-3">{{ $reports->links() }}</div>
                @endif
            </div>
        </div>
    </div>
