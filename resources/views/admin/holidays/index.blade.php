...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-stone-800">Master Data — Hari Libur</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-4xl space-y-6 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.holidays.store') }}"
                ...
                <div class="sm:col-span-2">
                    <label class="mb-1 block text-xs font-medium text-stone-500">Keterangan</label>
            ...
            <form method="GET" class="flex items-end gap-3 rounded-lg bg-white p-5 shadow-sm">
                <div>
                    <label class="mb-1 block text-xs font-medium text-stone-500">Tahun</label>
                    <select name="year" class="rounded-md border-stone-300 text-sm focus:border-kua-500 focus:ring-kua-500">
                        @foreach ($years as $y)
                            <option value="{{ $y }}" @selected($year == $y)>{{ $y }}</option>
                        @endforeach
                    </select>
                </div>
                <button class="rounded-md bg-stone-800 px-4 py-2 text-sm font-semibold text-white hover:bg-stone-700">Tampilkan</button>
            </form>
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <table class="min-w-full divide-y divide-stone-200 text-sm">
                    ...
                    <tbody class="divide-y divide-stone-100">
                        @forelse ($holidays as $holiday)
                            <tr class="hover:bg-stone-50">
                                ...
                                <td class="px-5 py-3">
                                    ...
                                    <div class="mt-0.5 text-xs text-stone-400">{{ $holiday->formatted_date }}</div>
                                </td>
                                <td class="px-5 py-3">
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-10 text-center text-stone-500">Belum ada hari libur di tahun {{ $year }}.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
