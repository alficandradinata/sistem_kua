...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">WhatsApp</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-5xl space-y-6 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div class="rounded-lg bg-white p-5 shadow-sm">
                    ...
                    <p class="mt-1 font-semibold text-gray-900">{{ $driverName }}</p>
                    @if ($driver !== 'cloud')
                        <p class="mt-2 text-xs text-yellow-700">
                            Mode uji coba — pesan tidak benar-benar dikirim ke WhatsApp.
                        </p>
                    @elseif (! $enabled)
                        <p class="mt-2 text-xs text-red-700">Kanal dimatikan (WHATSAPP_ENABLED=false).</p>
                    @else
                        <p class="mt-2 text-xs text-green-700">Terhubung ke Meta.</p>
                    @endif
                </div>
                <div class="rounded-lg bg-white p-5 shadow-sm">
                    <p class="text-sm text-gray-500">Pesan masuk / keluar</p>
                    <p class="mt-1 text-3xl font-bold text-gray-900">{{ $inboundCount }} / {{ $outboundCount }}</p>
                    @if ($failedCount > 0)
                        <p class="mt-2 text-xs text-red-700">{{ $failedCount }} gagal terkirim.</p>
                    @endif
                </div>
                <div class="rounded-lg bg-white p-5 shadow-sm">
                    <p class="text-sm text-gray-500">Nomor WA KUA</p>
                    <p class="mt-1 font-semibold text-gray-900">
                        {{ $contactNumber ? \App\Support\PhoneNumber::format($contactNumber) : 'Belum diatur' }}
                    </p>
                    <p class="mt-2 text-xs text-gray-500">Dipakai tombol "Chat WhatsApp" di halaman publik.</p>
                </div>
            </div>

            {{-- Daftar periksa kredensial + webhook --}}
            <div class="rounded-lg bg-white p-5 shadow-sm">
                <h3 class="mb-3 font-semibold text-gray-800">Kesiapan Sambungan</h3>
                <ul class="space-y-1 text-sm">
                    @foreach ($kredensial as $label => $terisi)
                        <li class="flex items-center gap-2">
                            <span class="{{ $terisi ? 'text-green-600' : 'text-gray-300' }}">{{ $terisi ? '✓' : '○' }}</span>
                            <span class="{{ $terisi ? 'text-gray-700' : 'text-gray-400' }}">{{ $label }}</span>
                        </li>
                    @endforeach
                </ul>

                <div class="mt-4 rounded-md border border-blue-200 bg-blue-50 p-3 text-xs text-blue-800">
                ...
                <form method="POST" action="{{ route('admin.whatsapp.test') }}" class="mt-4 flex flex-wrap items-end gap-3">
                    @csrf
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500">Kirim pesan uji ke nomor</label>
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="border-b border-gray-100 px-5 py-3">
                    <h3 class="font-semibold text-gray-800">Balasan Otomatis</h3>
                ...
                <form method="POST" action="{{ route('admin.whatsapp.replies.store') }}"
                      class="grid gap-4 border-b border-gray-100 bg-gray-50 p-5 sm:grid-cols-6">
                    @csrf
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-xs font-medium text-gray-500">Kata kunci</label>
                        <input type="text" name="keyword" value="{{ old('keyword') }}" required maxlength="100"
                               placeholder="mis. syarat nikah"
                               class="w-full rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500">Pencocokan</label>
                        <select name="match_type" class="w-full rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500">Urutan</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" max="9999"
                               class="w-full rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div class="sm:col-span-2 sm:row-span-2">
                        <label class="mb-1 block text-xs font-medium text-gray-500">Isi balasan</label>
                        <textarea name="reply_body" rows="3" required maxlength="1000"
                                  class="w-full rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('reply_body') }}</textarea>
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                                ...
                                <td class="px-5 py-3 align-top">
                                    ...
                                    <div class="mt-2 flex gap-2">
                                        ...
                                        <input form="ar-{{ $reply->id }}" type="number" name="sort_order"
                                               ...
                                               class="w-20 rounded-md border-gray-300 text-xs focus:border-indigo-500 focus:ring-indigo-500">
                                    </div>
                                </td>
                                <td class="px-5 py-3 align-top">
                                    <textarea form="ar-{{ $reply->id }}" name="reply_body" rows="3" maxlength="1000"
                                              class="w-full rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ $reply->reply_body }}</textarea>
                                </td>
                                <td class="px-5 py-3 align-top">
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-10 text-center text-gray-500">Belum ada balasan otomatis.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 px-5 py-3">
                    <h3 class="font-semibold text-gray-800">20 Pesan Terakhir</h3>
                    <a href="{{ route('petugas.whatsapp.index') }}" class="text-xs text-indigo-600 hover:underline">
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($messages as $message)
                            <tr class="hover:bg-gray-50">
                                <td class="px-5 py-3 whitespace-nowrap text-gray-500">{{ $message->time_ago }}</td>
                                <td class="px-5 py-3">
                                    <x-status-badge
                                        :color="$message->is_inbound ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'"
                                        :label="$message->is_inbound ? 'Masuk' : 'Keluar'" />
                                </td>
                                <td class="px-5 py-3 whitespace-nowrap">
                                    {{ $message->formatted_number }}
                                    @if ($message->user)
                                        <div class="text-xs text-gray-400">{{ $message->user->name }}</div>
                                    @endif
                                </td>
                                <td class="px-5 py-3 text-gray-700">{{ \Illuminate\Support\Str::limit($message->body, 90) }}</td>
                                <td class="px-5 py-3">
                                    @if ($message->status === \App\Models\WhatsAppMessage::STATUS_FAILED)
                                        <span class="text-xs text-red-700" title="{{ $message->error }}">Gagal</span>
                                    @else
                                        <span class="text-xs text-gray-500">{{ $message->status }}</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-5 py-10 text-center text-gray-500">Belum ada lalu lintas pesan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
