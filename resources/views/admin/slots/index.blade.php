...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-stone-800">Master Data — Slot Antrean</h2>
    </x-slot>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-6xl space-y-6 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.slots.store') }}"
                ...
                <div>
                    <label class="mb-1 block text-xs font-medium text-stone-500">Jam mulai</label>
                    <input type="time" name="slot_start_time" value="{{ old('slot_start_time', '08:00') }}" required
                           class="w-full rounded-md border-stone-300 text-sm focus:border-kua-500 focus:ring-kua-500">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-stone-500">Durasi (mnt)</label>
                    <input type="number" name="slot_duration" min="5" max="240" step="5"
                           value="{{ old('slot_duration', 60) }}" required
                           class="w-full rounded-md border-stone-300 text-sm focus:border-kua-500 focus:ring-kua-500">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-stone-500">Kuota/hari</label>
            ...
            <form method="GET" class="flex items-end gap-3 rounded-lg bg-white p-5 shadow-sm">
                <div>
                    <label class="mb-1 block text-xs font-medium text-stone-500">Saring per layanan</label>
                    <select name="service_id" class="rounded-md border-stone-300 text-sm focus:border-kua-500 focus:ring-kua-500">
                ...
                <button class="rounded-md bg-stone-800 px-4 py-2 text-sm font-semibold text-white hover:bg-stone-700">Terapkan</button>
                <a href="{{ route('admin.slots.index') }}" class="py-2 text-sm text-stone-500 hover:text-stone-800">Reset</a>
            </form>
                        ...
                        <tbody class="divide-y divide-stone-100">
                            @forelse ($slots as $slot)
                                <tr class="hover:bg-stone-50">
                                    ...
                                    <td class="px-5 py-3 font-medium text-stone-900">{{ $slot->service->name }}</td>
                                    <td class="px-5 py-3">
                                        <input form="slot-{{ $slot->id }}" type="time" name="slot_start_time"
                                               value="{{ substr($slot->slot_start_time, 0, 5) }}"
                                               class="w-28 rounded-md border-stone-300 text-sm focus:border-kua-500 focus:ring-kua-500">
                                        <div class="mt-0.5 text-xs text-stone-400">s/d {{ substr($slot->slot_end_time, 0, 5) }}</div>
                                    </td>
                                    <td class="px-5 py-3">
                                        <input form="slot-{{ $slot->id }}" type="number" name="slot_duration" min="5" max="240" step="5"
                                               value="{{ $slot->slot_duration }}"
                                               class="w-20 rounded-md border-stone-300 text-sm focus:border-kua-500 focus:ring-kua-500">
                                    </td>
                                    <td class="px-5 py-3">
                            ...
                            @empty
                                <tr><td colspan="6" class="px-5 py-10 text-center text-stone-500">Belum ada slot.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
