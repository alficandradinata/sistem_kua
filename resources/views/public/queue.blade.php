...
<body class="min-h-screen bg-kua-950 font-sans text-white antialiased">
    ...
    <div class="mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-8">
        ...
        <header class="flex flex-wrap items-baseline justify-between gap-3 border-b border-white/10 pb-5">
            ...
            <p class="text-base text-kua-200 sm:text-lg">
        ...
        <section class="flex flex-1 flex-col items-center justify-center py-10">
            <p class="text-lg uppercase tracking-[0.25em] text-emas-300 sm:text-xl">Nomor Dipanggil</p>

            @if ($current)
                <p class="mt-4 font-display text-[6rem] font-semibold leading-none tracking-tight text-white sm:text-[10rem]">
                    {{ $current->queue_number }}
                </p>
                <p class="mt-5 text-lg text-kua-200 sm:text-xl">
                    Silakan menuju loket pelayanan
                </p>
            @else
                <p class="mt-4 font-display text-[6rem] font-semibold leading-none text-kua-800 sm:text-[10rem]">—</p>
                <p class="mt-5 text-lg text-kua-200 sm:text-xl">
        ...
        <section class="border-t border-white/10 pt-6">
            <p class="text-sm uppercase tracking-[0.15em] text-kua-200">Nomor Berikutnya</p>

            @if ($next->isEmpty())
                <p class="mt-3 text-lg text-kua-300">Tidak ada nomor yang menunggu.</p>
            @else
                <div class="mt-3 flex flex-wrap gap-3">
                    @foreach ($next as $queue)
                        <span class="rounded-xl bg-white/10 px-5 py-3 font-display text-2xl font-semibold tracking-tight sm:text-3xl">
                            {{ $queue->queue_number }}
                        </span>
                    @endforeach
                </div>
        ...
        <footer class="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5
                       text-base text-kua-200 sm:text-lg">
            ...
            <span class="text-sm text-kua-300">
